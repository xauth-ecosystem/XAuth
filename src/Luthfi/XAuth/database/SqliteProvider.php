...
class SqliteProvider implements DataProviderInterface {
    ...
    public function __construct(Main $plugin) {
        ...
        $this->db->exec("CREATE TABLE IF NOT EXISTS players (name TEXT PRIMARY KEY, password TEXT, ip TEXT)");
    }
    ...
    public function registerPlayer(Player $player, string $hashedPassword): void {
        ...
        $stmt = $this->db->prepare("INSERT INTO players (name, password, ip, locked, registered_at, registration_ip) VALUES (:name, :password, :ip, 0, :registered_at, :registration_ip)");
        $stmt->bindValue(":registered_at", time(), SQLITE3_INTEGER);
        $stmt->bindValue(":registration_ip", $player->getNetworkSession()->getIp(), SQLITE3_TEXT);
        $stmt->bindValue(":name", $name, SQLITE3_TEXT);
    ...
    public function isPlayerLocked(string $playerName): bool {
        ...
        return $row !== false && (bool)$row['locked'];
    }
